/**
 * Vincular Criativos — Campanha 1 Video Views HR Andrade
 *
 * Para cada conjunto: baixa C1, C2, C3 do Drive → upload Meta → AdCreative → Ad
 * CTA: LEARN_MORE → humbertoandrade.com.br
 */
...
const ADSETS = [
  ...
  {
    id: '120248466252030460',
    procedimento: 'Otoplastia',
    folderId: '1GbYCKK_vUdWX2F_uKuHMLnPzVxjangiQ',
    copy: {
      message: 'A posição das orelhas pode ser corrigida com precisão e resultado natural.\n\nOtoplastia no Instituto HR Andrade — cirurgia rápida, recuperação tranquila.',
      headline: 'Corrija as orelhas com segurança',
      link: 'https://www.humbertoandrade.com.br',
    },
  },
  {
...
async function uploadVideo(filePath, name) {
  ...
  return videoId; // usar mesmo que ainda processando
}
...
// ─── MAIN ─────────────────────────────────────────────────────────────────────
async function main() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hr-ads-'));
  const results = [];

  console.log('════════════════════════════════════════════════════════════');
  console.log('  Vinculando Criativos — Campanha 1 Video Views');
  console.log('  HR Andrade Instituto | 4 conjuntos x 3 criativos = 12 ads');
  console.log('════════════════════════════════════════════════════════════');
  ...
  for (const adset of ADSETS) {
    ...
    const driveFiles = await listDriveVideos(adset.folderId);
    console.log(`  ${driveFiles.length} vídeos encontrados no Drive`);
