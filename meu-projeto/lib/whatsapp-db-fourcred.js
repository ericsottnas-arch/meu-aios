// meu-projeto/lib/whatsapp-db-fourcred.js
// Banco separado: Fourcred

const Database = require('better-sqlite3');
const path = require('path');
const fs = require('fs');

const DB_PATH = path.resolve(__dirname, '../../../docs/clientes/fourcred/banco-dados/conversas.db');
let db = null;
...
function initDB() {
  ...
  console.log(`✅ Fourcred DB inicializado em ${DB_PATH}`);
}
...
function saveMessage(parsed) {
  ...
  try {
    ...
    console.log(`   [DB-FOURCRED] Inserido: ${result.changes} linhas`);
  } catch (err) {
    console.error(`   [DB-FOURCRED] ❌ Erro ao salvar:`, err.message);
  }
}
