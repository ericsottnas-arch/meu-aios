// meu-projeto/lib/whatsapp-db-cleugo.js
// Banco separado: Dr. Cleugo

const Database = require('better-sqlite3');
const path = require('path');
const fs = require('fs');

const DB_PATH = path.resolve(__dirname, '../../../docs/clientes/dr-cleugo/banco-dados/conversas.db');
let db = null;
...
function initDB() {
  ...
  console.log(`✅ Dr. Cleugo DB inicializado em ${DB_PATH}`);
}
...
function saveMessage(parsed) {
  ...
  try {
    ...
    console.log(`   [DB-CLEUGO] Inserido: ${result.changes} linhas`);
  } catch (err) {
    console.error(`   [DB-CLEUGO] ❌ Erro ao salvar:`, err.message);
  }
}
