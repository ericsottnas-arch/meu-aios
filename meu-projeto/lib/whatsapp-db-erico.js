// meu-projeto/lib/whatsapp-db-erico.js
// Banco separado: apenas Dr. Erico Servano (instância comercial)

const Database = require('better-sqlite3');
const path = require('path');
const fs = require('fs');

const DB_PATH = path.resolve(__dirname, '../../../docs/clientes/dr-erico-servano/banco-dados/conversas.db');
let db = null;
...
function initDB() {
  ...
  console.log(`✅ Dr. Erico DB inicializado em ${DB_PATH}`);
}
...
function saveMessage(parsed) {
  ...
  try {
    ...
    console.log(`   [DB-ERICO] Inserido: ${result.changes} linhas`);
  } catch (err) {
    console.error(`   [DB-ERICO] ❌ Erro ao salvar:`, err.message);
  }
}
