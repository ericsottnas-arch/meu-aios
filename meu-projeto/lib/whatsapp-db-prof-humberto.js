// meu-projeto/lib/whatsapp-db-prof-humberto.js
// Banco separado: Prof. Dr. Humberto Andrade

const Database = require('better-sqlite3');
const path = require('path');
const fs = require('fs');

const DB_PATH = path.resolve(__dirname, '../../../docs/clientes/prof-dr-humberto-andrade/banco-dados/conversas.db');
let db = null;
...
function initDB() {
  ...
  console.log(`✅ Prof. Dr. Humberto Andrade DB inicializado em ${DB_PATH}`);
}
  ...
  try {
    const stmt = db.prepare(`
      INSERT OR IGNORE INTO conversas (
        ...
        message_type, is_from_me, is_group, timestamp, audio_url, audio_duration, transcription, urgency_score
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    console.log(`   [DB-PROF-HUMBERTO] Inserido: ${result.changes} linhas`);
  } catch (err) {
    console.error(`   [DB-PROF-HUMBERTO] ❌ Erro ao salvar:`, err.message);
  }
}
...
function getStats(chatJid) {
  ...
  const stmt = db.prepare(`
    SELECT
      ...
      AVG(urgency_score) as urgencia_media
    FROM conversas
    WHERE chat_jid = ?
  `);
