...
commands:
  ...
  # Client Onboarding
  - name: onboard-client
    args: '{client-name} {client-type} {specialty}'
    visibility: [full, quick, key]
    description: 'Onboard new client - create Google Drive folder structure'
...
whatsapp_intelligence:
  enabled: true
  description: Query WhatsApp conversations captured from Stevo webhook and stored in local SQLite
  capabilities:
    - list_all_chats
  ...
  database:
    ...
    path: meu-projeto/data/whatsapp-conversations.db
    tables:
      - conversas (message history with FTS5)
  modules:
    - lib/whatsapp-db.js (core database functions)
    - scripts/sync-supabase-to-sqlite.js (one-time sync from Supabase)
    - scripts/pm-whatsapp-cli.js (CLI for PM agent)
  cli_commands:
    chats: 'node scripts/pm-whatsapp-cli.js chats'
    conversation: 'node scripts/pm-whatsapp-cli.js conversation <jid> [limit]'
    search: 'node scripts/pm-whatsapp-cli.js search "<query>"'
    summarize: 'node scripts/pm-whatsapp-cli.js summarize <phone>'
    brief: 'node scripts/pm-whatsapp-cli.js brief "<client-name>"'
...
Query client conversations captured from WhatsApp and analyze engagement patterns.

### Commands

- `*chats` - List all WhatsApp chats with latest messages and message counts
- `*conversation {chat-jid} [limit]` - View conversation history (default: 20 messages)
- `*search "{query}"` - Full-text search across all messages
- `*summarize {phone}` - View client statistics (total messages, engagement rate, timeline)
- `*whatsapp-brief "{client-name}"` - Generate comprehensive client briefing with engagement insights

### Use Cases

- **Client onboarding** → Run `*whatsapp-brief` to understand communication history
- **Project planning** → Check `*summarize` to see response patterns and engagement
- **Information recall** → Use `*search` to find specific discussions or decisions
- **Campaign prep** → Review `*conversation` before client calls to get context
- **Relationship management** → Monitor engagement trends with `*summarize`

### How It Works

1. **Real-time capture** → Nico (WhatsApp agent) captures messages via Stevo webhook
2. **Dual storage** → Messages saved to Supabase + local SQLite simultaneously
3. **Local query** → PM agent queries SQLite for instant access (no API latency)
4. **One-time sync** → First use: run `node scripts/sync-supabase-to-sqlite.js` to import history

### Database

- **Location**: `meu-projeto/data/whatsapp-conversations.db`
- **Type**: SQLite3 with FTS5 (full-text search)
- **Auto-sync**: New messages added automatically from webhook
- **Manual sync**: `node scripts/sync-supabase-to-sqlite.js` (imports Supabase history)
